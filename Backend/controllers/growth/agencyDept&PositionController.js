...
import {
  ...
  CEO,
  CeoPosition,
} from "../../models/growth/agencyDept&PositionSchema.js";

///-------------OFFICE---------

const addOffice = async (req, res) => {
  const { ceo, name, status, weakness, strength } = req.body;
  ...
  // Fetch the Branch
  const ceoid = await CEO.findById(ceo);
  if (!ceoid) {
    return res
      .status(404)
      .send({ success: false, message: "CEO not found" });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Office created successfully", Office: office });
  } catch (error) {
  ...
  try {
    ...
    if (!ceoid) {
     ...
     if (SOP_filePath) {
      ...
      if (office.SOP_file && office.SOP_file.public_id) {
        await deleteFileFromCloudnary(office.SOP_file.public_id);
      }
      office.SOP_file = {
        secure_url,
        public_id,
...
const addCEO = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "CEO created successfully", CEO: ceo });
  } catch (error) {
    res.status(500).json({ message: "Error creating CEO", error });
  }
};
//update office
const updateCEO = async (req, res) => {
  try {
    ...
    if (description) Ceo.description = description;
    await Ceo.save();
    return res.status(200).send({
      success: true,
      message: "CEO Details Updated",
    });
    res.status(201)
      .json({ message: "CEO Updated successfully", CEO: Ceo });
  } catch (error) {
    res.status(500).json({ message: "Error creating CEO", error });
  }
};
...
const getCEO = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      success: true,
      total: ceo.length,
      message: "All CEO Fetch",
      ceo,
    });
  } catch (error) {
    res.status(500).json({ message: "Error fetching CEO", error });
  }
};
// Get a Office by ID
const getCEOById = async (req, res) => {
  try {
    const { ceoID } = req.params;
    const ceo = await CEO.findById(ceoID);
    if (!ceo) {
...
// Delete a Office by ID
const deleteCEO = async (req, res) => {
  try {
    const { ceoID } = req.params;
    const ceo = await CEO.findByIdAndDelete(ceoID);
    if (!ceo) {
      return res.status(404).json({ message: "CEO not found" });
    }
    res.status(200).json({ message: "CEO deleted successfully" });
  } catch (error) {
    res.status(500).json({ message: "Error deleting CEO", error });
  }
};
...
////---- POSITIONS-----------
//----------- DEPARTMENT POSITION--------
const addCeotposition = async (req, res) => {
  const {
    name,
    description,
    responsibilities,
  ...
  // Fetch the CEO
  const id = await CEO.findById(ceo);
  if (!id) {
    return res
      .status(404)
      .send({ success: false, message: "CEO not found" });
  ...
  try {
    const newPosition = new CeoPosition({
      name,
      description,
      SOP_file: {
        secure_url,
...
// Update a division position by ID
const updateCeoposition = async (req, res) => {
  try {
    const {
      name,
      description,
...
// Get all Division position
const getCeopositions = async (req, res) => {
  try {
    const position = await CeoPosition.find({});
    return res.status(200).send({
      success: true,
      total: position.length,
      message: "All position Fetch",
...
// Get a division position by ID
const getCeopositionById = async (req, res) => {
  try {
    const { positionID } = req.params;
    const position = await CeoPosition.findById(positionID).populate("ceo");
    if (!position) {
      return res.status(404).json({ message: "Position not found" });
    }
    res.status(200).json(position);
...
// Delete a Division position by ID
const deleteCeoposition = async (req, res) => {
  try {
    const { positionID } = req.params;
    const Position = await CeoPosition.findByIdAndDelete(positionID);
    if (!Position) {
      return res.status(404).json({ message: "Position not found" });
    }
    res.status(200).json({ message: "Position deleted successfully" });
